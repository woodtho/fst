...
import { getConsolidationBooklets, getConsolidationQuestionsForRange, getConsolidationItemsForRange, getObjectives, getItemsForOfRange } from "@/lib/content";
    ...
    <>
      ...
      <div className="grid">
        {booklets.map((b) => {
          ...
          const bookletCount = getConsolidationItemsForRange(from, to).length;
          ...
          return (
            <Link key={b.booklet} href={`/consolidation/${from}-${to}`} className="card">
              ...
              <div className="card-sub">{ofs.length} objectives · {bookletCount} booklet questions · {count} range practice questions · {sourceCount} answer-keyed activities · source {b.catalogue}</div>
